...
public class GetModifierGroupsQueryHandler : IRequestHandler<GetModifierGroupsQuery, ApiResponse>
{
    ...
    public GetModifierGroupsQueryHandler(IUnitOfWork<ShipCapstoneContext> unitOfWork, ILogger logger, IClaimService claimService)
    {
    ...
    public async ValueTask<ApiResponse> Handle(GetModifierGroupsQuery request, CancellationToken cancellationToken)
    {
        ...
        var modifierGroups = await _unitOfWork.GetRepository<ModifierGroup>().GetPagingListAsync(
            ...
            page: request.Page,
            size: request.Size,
            sortBy: request.SortBy ?? nameof(ModifierGroup.CreatedDate),
            isAsc: request.IsAsc
        );
